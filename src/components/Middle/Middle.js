...
function Middle() {
    ...
    const numberHandelerChange = (event) =>{
    ...
    const chackLpNumberHandeler = (event) => {
        ...
        setLoading(true);
        if(lpNumber){
            axios.get("https://data.gov.il/api/3/action/datastore_search?resource_id=053cea08-09bc-40ec-8f7a-156f0677aff3&q="+lpNumber)
                .then((res)=>{
                    setCarData(res.data.result.records[0])
                    axios.get("https://data.gov.il/api/3/action/datastore_search?resource_id=c8b9f9c8-4612-4068-934f-d4acd2e3c06e&q="+lpNumber)
                        .then((res)=>{
    ...
    const loading = isLoading && <div className={classes.Loader}>טוען...</div>;
    return (
        ...
        <div className={classes.Lp}>
            <input type="number" name="lpNumber" pattern="[0-9]*"  className={classes.Input} value={lpNumber} onChange={numberHandelerChange} />
